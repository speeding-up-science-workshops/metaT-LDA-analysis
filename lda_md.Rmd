...
```{r import samples}
count<-read.table(file = "~/Desktop/LDA-binder/KEGG_module_TPM.tsv", sep = '\t', header = TRUE, row.names=1)
...
```{r melt}
count<-read.csv("~/Desktop/LDA-binder/KEGG_module_TPM.csv", header=T, row.names=1)
count<-t(count)
count
